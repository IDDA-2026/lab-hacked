...
public class PostController {
    ...
    public List<SearchResult> search(@RequestParam(name = "q", defaultValue = "") String q) {
        // Используем встроенный безопасный метод репозитория, передавая строку поиска в оба параметра
        List<Post> posts = postRepository.findByTitleContainingIgnoreCaseOrBodyContainingIgnoreCase(q, q);

        // Преобразуем сущности Post в объекты SearchResult для фронтенда
        return posts.stream()
